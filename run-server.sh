...
find_node () {
  ...
  # Identify "left side junk" and use it to find the string in npm config that we're looking for
  ...
  # sed is a little weird thing. the syntax is `s/whattoreplace/withwhat/`, so it is barely readable when we
  ...
  # Third sed cleans up after the first - after initial s/, \/\/ means "substitute //", after which you got the "/"
...
find_puppeteer () {
  # Check with npm if puppeteer is installed locally and handle it like in function below
  # If it's not installed, install it.
  npmoutput=$(npm list | grep puppeteer)
  pkgname="puppeteer"
...
run_http_server () {
  ...
  pkgname="http-server"
  if [ $(expr index "$npmoutput" "$pkgname") = 0 ]; then # Look for absence of "http-server" in the junky string specifically
    ...
    si_echo "The screenshot HTTP server finished running at http://localhost:4444."
    read -p "" | si_echo "Press Enter to continue . . ."
  fi
}
...
find_puppeteer # This has to be ran before run_http_server due to the directory change
